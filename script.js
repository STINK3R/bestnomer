...
// Загрузка данных из Supabase (всегда заменяет локальные данные)
async function loadDataFromSupabase() {
    try {
        const result = await loadAllDataFromSupabase();
        
        masksData.length = 0;
        (result.masks || []).forEach(m => masksData.push(m));
        
        numbersData.length = 0;
        (result.numbers || []).forEach(n => numbersData.push(n));
        
        tariffsData.length = 0;
        (result.tariffs || []).forEach(t => tariffsData.push(t));
    } catch(e) {
        console.error('Error loading data from Supabase:', e);
    }
}

// Плавное появление сайта после загрузки данных
function revealSite() {
    ...
    if (mainContainer) {
        ...
        mainContainer.classList.add('site-revealed');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Загружаем данные из Supabase и рендерим
...
function updateTariffsSlider() {
    ...
    if (!isMobile) {
        ...
        tariffsSlider.style.transition = 'transform 0.5s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
    } else {
...
function initScrollAnimations() {
    const animElements = document.querySelectorAll('.section-title, .feature-item, .mask-group, .reviews-link-btn, .operators-grid, .tariffs-wrapper, .pick-tariff-container');
    
    animElements.forEach(el => {
        el.classList.add('animate-on-scroll');
    });
    
    // Каскадные задержки для feature items
    document.querySelectorAll('.feature-item').forEach((item, i) => {
        item.classList.add(`animate-delay-${(i % 4) + 1}`);
    });
